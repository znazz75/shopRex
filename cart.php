...
// No shipping method is chosen yet at this point (that happens at
// checkout) - show the cheapest active method's cost as an estimate.
$cartWeightKg = Cart::getWeightKg();
$totalQuantity = Cart::count();
$shipping = null;
foreach (Cart::getActiveShippingMethods() as $method) {
    $cost = Cart::calculateShippingForMethod((int)$method['id'], $cartWeightKg, $subtotal, $totalQuantity);
    if ($shipping === null || $cost < $shipping) {
        $shipping = $cost;
    }
}
$shipping = $shipping ?? 0.0;
// Set by product.php on every product view; falls back to the shop
// homepage if the visitor hasn't looked at a product yet this session.
$continueShoppingUrl = $_SESSION['last_product_url'] ?? (rtrim(SITE_URL, '/') . '/index.php');
...
<?php if (empty($items)): ?>
  <p class="text-secondary"><?= e(__('cart.empty')) ?> <a href="<?= rtrim(SITE_URL, '/') ?>/index.php"><?= e(__('common.continue_shopping')) ?></a>.</p>
<?php else: ?>
  <div class="row g-4">
    <div class="col-lg-8">
            ...
            <tbody>
              <?php foreach ($items as $item): ?>
                <tr>
                  ...
                  <td>
                    <a class="text-body fw-semibold text-decoration-none" href="<?= rtrim(SITE_URL, '/') ?>/product.php?slug=<?= e($item['slug']) ?>"><?= e($item['name']) ?></a>
      ...
      <?php foreach ($items as $item): ?>
        <form id="remove-<?= e($item['key']) ?>" method="post" action="<?= rtrim(SITE_URL, '/') ?>/cart_action.php" data-confirm="<?= e(__('cart.confirm_remove')) ?>">
          <?= csrfField() ?>
      ...
      <div class="card">
        <div class="card-body">
          ...
          <div class="d-flex justify-content-between mb-2">
            <span><?= e(__('common.shipping')) ?><?= count(Cart::getActiveShippingMethods()) > 1 ? ' (' . e(__('cart.shipping_from')) . ')' : '' ?></span>
          ...
          <a class="btn btn-primary w-100" href="<?= rtrim(SITE_URL, '/') ?>/checkout.php"><?= e(__('cart.proceed_to_checkout')) ?></a>
        </div>
      </div>
    </div>
  </div>
