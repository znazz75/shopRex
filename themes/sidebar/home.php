 ...
 * (see includes/home.php for the default this replaces, and
 * themeTemplatePath() in includes/functions.php for how it's selected).
 * Structurally different from the default: a persistent category tree in a
 * left sidebar instead of a top breadcrumb + subcategory-chip row. Every
 * variable used below is set by index.php before this file is required.
 */
function shopRexSidebarRenderCategoryTree(array $nodes, ?int $activeCategoryId, array $activeChainIds): void
{
    ...
    foreach ($nodes as $node) {
        ...
        echo '<li>';
        echo '<a href="' . rtrim(SITE_URL, '/') . '/index.php?category=' . (int)$node['id'] . '"'
            . ' class="' . ($isActive ? 'active' : '') . '">' . e($node['name']) . '</a>';
        if (!empty($node['children']) && ($isInChain || $isActive)) {
            shopRexSidebarRenderCategoryTree($node['children'], $activeCategoryId, $activeChainIds);
        }
        echo '</li>';
    }
    echo '</ul>';
...
<div class="row g-4">
  <aside class="col-md-3 sidebar-categories">
    ...
    <ul class="sidebar-category-tree">
      <li><a href="<?= rtrim(SITE_URL, '/') ?>/index.php" class="<?= !$categoryId ? 'active' : '' ?>"><?= e(__('shop.all_products')) ?></a></li>
    </ul>
    <?php
    $activeChainIds = array_map(fn($c) => (int)$c['id'], $categoryPath);
    shopRexSidebarRenderCategoryTree(getCategoryTree(), $categoryId, $activeChainIds);
    ?>
  </aside>
