<?php
/**
 * Default product-listing content (index.php - homepage, category, and
 * search-within-category views all share this one route/template). This
 * is the "home.php" template slot a theme package (themes/<key>/home.php)
 * can override for a structurally different layout - see
 * themeTemplatePath() in includes/functions.php and README.md's "Frontend
 * theme" section. index.php itself only fetches data; every variable used
 * below ($categoryPath, $categoryId, $subcategories, $products, etc.) is
 * set there before this file is required.
 */
?>
<?php if ($categoryPath): ?>
...
<?php if (!empty($subcategories)): ?>
  <div class="subcategory-chips d-flex flex-wrap gap-2 mb-4">
    <?php foreach ($subcategories as $sub): ?>
      <a class="btn btn-sm btn-outline-secondary" href="<?= rtrim(SITE_URL, '/') ?>/index.php?category=<?= (int)$sub['id'] ?>"><?= e($sub['name']) ?></a>
    <?php endforeach; ?>
  </div>
<?php endif; ?>
